...
class ColorRowWithPicker extends Component {
    ...
    render() {
        let {showColorPickerModal, color} = this.state;
        return (
          <>
            <tr>
                <td>
                    <div class="pick-color">
                        <span style={{background: color}} onClick={() => this.setState({showColorPickerModal: true})}></span>
                        <input type="text" placeholder="Pick color" onChange={(e) => this.setState({color: e.target.value})} value={color} class="bg-gray-light border-0"/>
                    </div>
                </td>
                <td>
                    <input type="text" placeholder="Enter color name" class="bg-gray-light border-0 pick-color"/>
                </td>
                <td align="right">
