...
const PODetails = () => {
                ...
                <div className='colors-row'>
                    <div className='multicolors'>
                        <span className='color-icon'>
                            <img
                                src={color?.swatchDocResponse?.docUrl}
                                alt={'swatch color'}
                            />
                        </span>
                    </div>
    ...
    const renderColorWiseList = (order) => {
        return (
            <table className='table'>
                ...
                <tbody>
                    {order?.colorWiseSizeQuantityPairList &&
                        order.colorWiseSizeQuantityPairList.map(
                            (value, index) => {
                                let qty = 0
                                return (
    ...
    const renderOrderList = () => {
        return orderList?.map((order, index) => {
            return (
                <div
                    className={`single-design flex bg-white ${uniqueStyleName(
                        order
                    )}`}
                    key={`po_order_${index}`}
                >
                    <div
                        ...
                        onClick={() => redirectToDesignView(order.productId)}
                    >
                        <img
                            src={order?.documentResponseList[0]?.docUrl}
                            alt=''
                        />
                    </div>
                    <div className='design-details-info'>
                        <div className='design-title mt-3 flex justify-content-between'>
                            <h3
                                className='text-base font-semibold mb-0'
                                onClick={() =>
                                    redirectToDesignView(order.productId)
                                }
                            >
                                <Tooltip
                                    title={order.name}
                        ...
                        <div className='po-details-view'>
                            ...
                            <div className='po-quantity-colums-view'>
                                <ul>
                                    ...
                                    <li>
                                        <span>
                                            {
                                                order
                                                    ?.colorWiseSizeQuantityPairList[0]
                                                    ?.sizeQuantityPairList
                                                    ?.length
                                            }
                                        </span>{' '}
                                        size
                                ...
                                <div className='values flex mt-1'>
                    ...
                    <div className='design-quantity-table mt-2      '>
                        <div className='text-right mb-2'>
                            <p className='regular-14'>
                                ...
                                {order.priceType ? order.priceType : 'FOB'}
                            </p>
                        </div>
        ...
        <LoaderComponent loading={loader}>
            <div className='create-order-container'>
                <div className='designs-info-section'>
                    <h3 className='text-xl mb-5 flex'>
                        <img
