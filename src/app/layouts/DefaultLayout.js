...
const DefaultLayout = () => {
  ...
  const [showFavouriteSubmenu, setShowFavouriteSubmenu] = useState(false)
  ...
  const renderCollectionSubmenu = () => {
    if (showCollectionSubMenu) {
      return (
        <div
          className='submenu flex flex-col sm:flex-row p-5 bg-white shadow-lg w-[1240px] sm:h-[420px]  absolute top-[58px] left-[50%] translate-x-[-50%] z-[999]'>
  ...
  const renderFavouriteSubmenu = () => {
    if (showFavouriteSubmenu) {
      return (
        <div
          className='submenu flex flex-col sm:flex-row p-6 bg-white shadow-lg w-[455px] sm:h-[386px]  absolute top-[66px] right-0  z-[999]'>
          <div>
      ...
      <div className='container-fluid bg-primaryColor-shade-300'>
        <header className='bg-white py-5 px-5 logo flex items-center'>
          ...
          <div className='main-menu hidden xl:block ml-16 4xl:ml-28'>
            <ul>
              <li className={`text-base text-primaryColor uppercase inline-block mr-6 4xl:mr-10 5xl:mr-14 ${activeMenu === 'dashboard' ? 'active' : ''}`}>
                <Link to='/dashboard'>Home</Link>
              </li>
              <li className={`text-base text-primaryColor uppercase inline-block mr-6 4xl:mr-10 5xl:mr-14 ${activeMenu === 'moodboard' ? 'active' : ''}`}>
                <Link to='/moodboard'>Moodboard</Link>
              </li>
              <li className='text-base text-primaryColor uppercase inline-block mr-6 4xl:mr-10 5xl:mr-14 relative'>
                <a href='#' onClick={() => setShowCollectionSubMenu(prevState => !prevState)}>Collections</a>
              ...
              <li className={`text-base text-primaryColor uppercase inline-block mr-6 4xl:mr-10 5xl:mr-14 ${activeMenu === 'orders' ? 'active' : ''}`}>
          ...
          <div className='user-interaction ml-auto flex items-center gap-x-6'
               onClick={() => setShowFavouriteSubmenu(prevState => !prevState)}>
            <div
