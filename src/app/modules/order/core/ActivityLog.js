...
const ActivityLog = ({ activity, setLoader, index }) => {
        ...
        {timelineImages[0] && (
          <div className='single-one'>
            {fileTypeOne === 'IMAGE' || fileTypeOne === 'NO_FILE' ? (
              <a href={timelineImages[0]} target='_blank' rel='noreferrer'>
        ...
        {timelineImages[1] && (
          <div className='single-one'>
            {fileTypeTwo === 'IMAGE' || fileTypeTwo === 'NO_FILE' ? (
              <a href={timelineImages[1]} target='_blank' rel='noreferrer'>
  ...
  const handePageRoute = (e) => {
    ...
    if (isReplying === true || activity.actionType === 'COMPLETED_ORDER' || activity.actionType === 'STARTED_ORDER' || showTaskDetailsModal === true || element === 'button text __reply-button__') {
      ...
      navigate.push(`/designs/view/${activity?.secondaryActedUpon?.id}?openModal=true`)
