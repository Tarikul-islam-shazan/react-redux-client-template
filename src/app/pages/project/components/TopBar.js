...
const TopBar = ({ details, orderId, getProjectDetails }) => {
   ...
   }, [details.projectMemberResponseList, getUsersByTypes]);
   ...
   const removeUserFromOrder = async (user) => {
      ...
      await Http.POST("removeMemberFromOrder", [body], "/" + orderId)
         .then(({ data }) => {
            if (data && data.success) {
               if (userResponseList && userResponseList.length) {
                  newUserResponseList = userResponseList.filter((item) => item.id !== user.id);
               }
   ...
   return (
      <div>
                     ...
                     <div className="position-relative">
                        <div class="added-members">
                           <div
                              ...
                              className={`add-new-member ${showAddMemberModal ? `show` : ``}`}
                           >
                              <svg
                     ...
                     <div className="delivery-status">
                        ...
                        <div className="date-details delivery-date-details text-left">
                           <ul className="start">
                              <li>Start:</li>
                              <li>
                                 {changeDateFormat(details.startDate, "DD-MM-YYYY", "DD MMM, YYYY")}
                              </li>
                           </ul>
                           <ul className="end">
                              <li>End:</li>
                              <li>
                                 {changeDateFormat(details.endDate, "DD-MM-YYYY", "DD MMM, YYYY")}
                              </li>
                           </ul>
                           <ul className="spent-status">
                              <li>Days spent:</li>
                              <li>
                                 <span className="time-spent">{details.daySpent}</span> Days spent
                              </li>
                           </ul>
                        </div>
                     </div>
                  ...
                  <div className="col-6 pl-0">
                     <ul className="d-flex py-5">
                        <li style={{ position: "relative" }} className="active">
                           <span onClick={onStyleClick}>All styles ({details.totalStyles})</span>
                        </li>
                        <li>{details.orderQuantity} units</li>
                        <li>${doCommaSeparationWithIntegers(details.totalValuation)}</li>
                     </ul>
                  </div>
         ...
         {showAllStyles && (
            <div
               className="all-style-absolute-section"
               ref={AllStylesModal}
            >
