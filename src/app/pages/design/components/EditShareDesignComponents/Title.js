...
export const Title = ({data, flag, flagName, toggleFlag, onChange, onSubmit}) => {  
  if (flag) {
    return (
      <div class="name">
           <div class="name-display d-flex justify-content-between align-items-center">

              <div class="edit-design-name w-100">
                    <input type="text" placeholder="Enter design name" class="bg-gray-light" name="name" value={data.name} onChange={(e) => onChange(e.target.name, e.target.value)}/>
                    <span class="done cursor-pointer" onClick={() => onSubmit(flagName)}>
  ...
  return(
    <div class="name">
         <div class="name-display d-flex justify-content-between align-items-center">
            ...
            <h4 class="font-26 semibold">{data.name}</h4>
