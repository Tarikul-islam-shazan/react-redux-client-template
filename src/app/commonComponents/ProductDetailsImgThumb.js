...
export const ProductDetailsImgThumb = ({item, index, showGallery, docs}) => {
  ...
  if (IMAGE_EXTS.includes(extension)) {
    return <a href={item} className="ref-file mr-2" target="_blank" download><img src={addImageSuffix(item, '_xthumbnail')}/></a>
  }
  return(
    <div className="ref-file mr-2 download-file text-center d-flex align-items-center justify-content-center">
        <a href={item} className="ellipsis" target="_blank" download>
            <div>
                ...
                <span className="ellipsis w-100 d-block mt-2 font-12">{item}</span>
            </div>
        </a>
    </div>
  )
